...
func ParseURL(pageURL string) (models.PageURLInfo, error) {
	...
	if pageURL == "" {
		return models.PageURLInfo{}, fmt.Errorf("URL is empty")
	}

	// Parse the URL
	u, err := url.Parse(pageURL)
	if err != nil {
		return models.PageURLInfo{}, fmt.Errorf("invalid URL: %w", err)
	}
	...
	if pageID == "" {
		return models.PageURLInfo{}, fmt.Errorf("could not extract page ID from URL")
	}
